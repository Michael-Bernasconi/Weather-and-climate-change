...
After the work done both on knowledge and data layer, we decided to modify some aspects of the previous version of the ER model: for the \textit{WeatherStation} entity we removed the \textit{shortname} attribute, as it was useless. Then, we added \textit{City} attribute directly in the \textit{Anomaly} entity: this modify has been done to link directly the anomaly with the city in which it has been detected. 