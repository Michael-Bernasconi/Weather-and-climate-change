...
\hline
Giulia Ferrari & 2.1 & What are the typical temperature ranges and humidity levels in alpine valleys compared to lake areas? \\ Giulia Ferrari & 2.2 & Which areas show microclimatic differences despite geographical proximity? \\ Giulia Ferrari & 2.3 & How does wind speed and direction vary between the Adige Valley and surrounding mountains areas? \\ Giulia Ferrari & 2.4 & Are there correlations between altitude and average annual temperature or humidity? \\ Giulia Ferrari & 2.5 & How stable are microclimates across seasons (winter vs. summer) in different valleys? \\ Giulia Ferrari & 2.6 & Which valleys show the most distinctive microclimatic patterns according to sensor data? \\
\hline
...
This section presents an overview about available information for this project. The information is used also to define knowledge concepts and to retrive data for populating the Knowledge Graph.
...
\begin{itemize}
    ...
    \item \textbf{\href{http://sweetontology.net/sweetAll}{SWEET Ontology}} (Semantic Web for Earth and Environmental Terminology) is a modular and extensible collection of ontologies, maintained by the Earth Science Information Partners (ESIP), covering a vast range of Earth system science concepts, including the atmosphere, hydrosphere, solid earth, and associated physical properties (such as time, space, and units of measure).
    In the context of this project, SWEET is leveraged to provide the rigorous scientific grounding required for modeling complex climatic aspects and concepts. 
    \item \textbf{\href{https://www.w3.org/TR/owl-time/}{Time Ontology in OWL (OWL-Time)}} is a W3C candidate recommendation that provides a vocabulary for expressing semantic relations between instants and intervals, as well as information about durations and date-time information.
    While standard datatypes (like \textit{xsd:dateTime}) provide simple timestamps, OWL-Time is leveraged to model complex temporal concepts essential for climate analysis.
    This ontological structure allows for advanced temporal reasoning, such as determining if a specific report falls within a particular seasonal interval.
...
\begin{itemize}
\item \textbf{\href{https://dati.trentino.it/dataset/observations-site-list}{Open Data Trentino}}: provides a comprehensive list of meteorological stations in the Trentino region. The dataset includes information such as station location, altitude, and available measurements, which can be used to analyze local weather conditions over time. The data is freely accessible and can be integrated into applications for environmental monitoring and weather analysis. For each station, the information indicated are:
\begin{itemize}
    \item \textbf{Code}: an identifier for each weather station.
    \item \textbf{Name}
    \item \textbf{Shortname}
    \item \textbf{Elevation}
    \item \textbf{Latitude and Longitude}: identify the location of the weather station.
    \item \textbf{Start and end date}: indicate from which date the weather station is working and, in some cases, the date in which it has stopped. 
\end{itemize}
Weather station data are essential to link meteorological observations from multiple sources to precise geographic locations, supporting location-based analyses and anomaly detection tasks.
\item \textbf{\href{https://www.ilmeteo.it/portale/archivio-meteo/Trento}{ilMeteo.it}}:  provides access to historical weather data for Trento from 1973 to 2025. For each year, there are historical data for each day of months. The main attributes for the weather reports are:
    \begin{itemize}
        ...
        \item \textbf{Precipitation (float)}: this data is particularly relevant to Scenario 1, to Scenario 3 and Scenario 5, but can also be useful for the others. 
        \item \textbf{Humidity rate (float)}: this is relevant for the Scenario 2, for the Scenario 3, for the Scenario 4 and can also helps to analyze trends for Scenario 5.
        \item \textbf{Max and average wind speed}: these are relevant for the Scenario 2 about microclimates and for Scenario 3.
        \item \textbf{Meteorological phenomena (string)}: it's required for queries about analysis of weather phenomena in time and also to individuate anomalies.
    \end{itemize}
    All the attributes indicated above are useful to answer the Competency Questions (CQs) defined in the previous section.
    \item \textbf{\href{https://open-meteo.com/en/docs/historical-weather-api}{Historical Weather API}}: it is based on reanalysis dataset and use of weather station, aircraft, buoy, radar and satellite observations to create a record of past weather conditions. The data is available for any location worldwide. The datasets used are: ECMWF IFS, ERA5, ERA5-Land, ERA5-Ensemble, CERRA, ECMWF IFS Assimilation Long Window. There is a set of meteorological variables available, both in hourly and daily granularity. Some of data are very similiar to \textit{ilMeteo.it} source, but there are some slightly differences that can help to analyze different aspects. The main attributes useful for this project are: 
    \begin{itemize}
        \item \textbf{Weather Code}: is a numerical  identifier used to identify weather phenomena. In this context, it's useful for queries about analysis of weather phenomena in time and also to individuate anomalies.
        \item \textbf{Mean, Maximum and Minimum temperature}: these data, like for ilMeteo.it source, are relevant for the majority of scenarios: in particular, for Scenario 2 as it helps to analyse microclimates, for Scenario 3 to track anomalies related to temperatures, Scenario 4 to track temperature changes and Scenario 5 for temperature long term trends. 
        \item \textbf{Precipitation, Rain and Snowfall sum and Precipitation Hours}: these data are particularly relevant to Scenario 1, to Scenario 3 and Scenario 5, but can also be useful for the others. 
        \item \textbf{Wind Direction, Gusts and Speed}: these are relevant for the Scenario 2 about microclimates and for Scenario 3, such as the data about wind gusts can help to individuate anomalies.
        \item \textbf{Mean, Maximum and Minimum Humidity}: these are necessary for the Scenario 2, for the Scenario 3, for the Scenario 4 and can also helps to analyze trends for Scenario 5.
    \end{itemize}
\end{itemize}
After collecting all the information sources, the ER model was modified in order to better represent the domain according to the available information. In particular, the PrecipitationReport was incorporated directly into the WeatherReport entity to better reflect the structure of the data sources. In addition, several attributes were added to selected entities following the analysis of the identified sources. The final result of these modifications is shown in Figure~\ref{fig:ER_model_final}.

\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{../../../../Phase 1 - Purpose Definition/ER_after_IG.png}
    \caption{ER model after the information gathering phase: precipitation information was integrated directly into the WeatherReport entity and some attributes were updated.}
    \label{fig:ER_model_final}
\end{figure}

The information gathering phase played a crucial role in understanding the available data and in assessing how well these data fit the defined personas and scenarios. The collected sources allow the modeling of all the purposes previously defined. Moreover, this phase was essential to refine the ER model in order to ensure a better alignment with the available data sources and ontologies.

Following the information gathering phase, the previously defined personas, scenarios, and Competency Questions (CQs) were reviewed. It was concluded that no modifications were required, as the available resources are sufficient to cover the entire domain of interest. Some data, such as seasons, microclimates, and anomalies, need to be derived from the available datasets. The procedures adopted to generate these data are described in the next section.