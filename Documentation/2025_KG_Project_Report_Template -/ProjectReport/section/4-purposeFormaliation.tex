...
\item \textbf{\href{https://www.ilmeteo.it/portale/archivio-meteo/Trento}{ilMeteo.it}}:  gives access to historical weather data for Trento from 1973 to 2025. For each year, there are historical data for each day of months. The main attributes for the weather reports are:
    ...
    \item \textbf{\href{https://open-meteo.com/en/docs/historical-weather-api}{Historical Weather API}}: it is based on reanalysis dataset and use of weather station, aircraft, buoy, radar and satellite observations to create a record of past weather conditions. The data is available for any location worldwide. The datasets used are: ECMWF IFS, ERA5, ERA5-Land, ERA5-Ensemble, CERRA, ECMWF IFS Assimilation Long Window. There is a set of meteorological variables available, both in hourly and daily granularity. There are several variables useful for this project, such as:
    \begin{itemize}
        \item Mean, Max an Min Temperature (float)
        \item Precipitation sum (float)
        \item Max wind speed(float)
        \item Mean, Max and Min relative Humidity (float)
        \item Snow sum (float)
        \item Weathercode (string)
        \item Soil Temperature (float)
    \end{itemize}
\end{itemize}

Given Knowledge and Data resources, it has been revaluated the initial personas, scenarios and competency questions, and the relative ER model precedently built. 
The precipitation aspect is well defined in the indicated ontologies and also has a lot of data across all the data sources. Form the previous ER, the precipitation report can be directly integrated in the weather report, given an analysis of the HW Ontology. The \textit{Giulia} scenario about microclimates can be derivated from historical weather data related to weather stations properties. The \textit{Alessandro} scenario can analyze 